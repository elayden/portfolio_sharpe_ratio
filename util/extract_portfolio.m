function [out] = extract_portfolio(csv_dir)
    ...
    for i = 1:length(data)
        returns = zeros(nRows,1);
        for j = 2:nRows
            returns(j) = (data{i}.Close(j) - data{i}.Close(j-1)) / data{i}.Close(j-1);
        end        
        out(i).ticker = tickers{i}; out(i).date = data{i}.Date(2:end);
        out(i).raw = data{i}.Close(2:end); out(i).return = returns(2:end);
        if sum(strcmpi(data{i}.Properties.VariableNames,'volume'))
